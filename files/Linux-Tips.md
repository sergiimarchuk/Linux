...
## Goal
Connect to a web application inside a Docker container on a remote server, when port 8080 is listening only on localhost of the server.
...
- **DESTINATION** — куди копіюємо (наприклад, `user@host:/path`)  
  **DESTINATION** — where to copy it (e.g., `user@host:/path`)
...
### **B. `my-tracker` (без слешу / without trailing slash)**
```bash
rsync -av my-tracker root@192.168.100.60:/opt/dev-py/tempo_Go/my-tracker
```
...
| Команда / Command                           | Що копіюється / What is copied               | Результат на сервері / Result on server                  |
|---------------------------------------------|-----------------------------------------------|----------------------------------------------------------|
...
- Слеш на DESTINATION зазвичай необов'язковий, але краще ставити для ясності.  
  Slash on DESTINATION is usually optional, but better to include for clarity.

---
---

# Linux Tips: Fixing Nautilus File Manager Issues
...
killall nautilus
```
Then try opening directories again. The file manager may freeze and need to be restarted.

### 2. Check Double-Click Settings
By default, you need to double-click folders to open them in the file manager. Make sure you're double-clicking, not single-clicking.
...
Some users found that WSL (Windows Subsystem for Linux) or the GNOME shell extension DING (desktop icons) caused issues where files and folders couldn't be opened. If you have desktop icons enabled, try disabling the desktop icons extension.
